...
let element = null;
let settings = null;
let activeHref = null; // Prepare to save the state
...
function setElement(callback) {
  try {
    if (IS_NEW_UI) {
      element = NEOS_DOC.querySelector("[target=neosPreview]").parentElement;
...
function addButton() {
  if (!NEOS_DOC.querySelector(`button.${NAMESPACE}.${settings.className}[title="${settings.title.replace(/' '/, '\ ')}"]`)) {
    ...
    button.innerHTML = settings.icon in ICONS ? ICONS[settings.icon] : settings.icon;
...
function buttonCallback(button, callback) {
  button = button ? button : element ? element.querySelector(`.${settings.className}`) : false;
...
const ButtonContextBar = {
  add: options => {
    if (options.className && options.icon && options.title && options.toggleClass) {
